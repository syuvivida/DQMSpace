...
class LumiDataPoint(object):
    ...
    def __init__(self, line, beginrun=-1, endrun=-1, json_file_name=None, data_scale_factor=1):
        ...
        #for recorded luminosity, only considered the ones in the run list
    
        #for recorded luminosity, only considered the ones in the run list
        #when one does not specify the run range because run_file is not available
        if (beginrun==-1) or (endrun==-1):
           self.lum_del = scale_factor * float(line_split[5])
           self.lum_rec = scale_factor * float(line_split[6])
#        if self.run_number in run_list:
        elif (self.run_number >= beginrun) and (self.run_number <= endrun):
           self.lum_del = scale_factor * float(line_split[5])
           self.lum_rec = scale_factor * float(line_split[6])
        else:
           self.lum_del = 0.
...
#                        lumicalc_flags += " --beamenergy %.0f --amodetag %s" % (beam_energy, accel_mode)
                        lumicalc_flags += " --amodetag %s" % (accel_mode)
...
#    print("Reading back brilcalc results")
    # Open the file that contains the runs to be processed
    run_file = cfg_parser.get("general", "run_file")
    if not os.path.exists(run_file):
        begin_run = -1
        end_run   = -1
    else:
        ListOfRuns = []
        with open(run_file) as txtfile:
            for line in txtfile:
                ListOfRuns.append(int(line.rstrip()))
        ListOfRuns.sort()
        print("The following runs are included")
        print(*ListOfRuns) 
        begin_run = min(ListOfRuns)
        end_run = max(ListOfRuns)

    print("Run range from run %d to %d" % (begin_run, end_run))
                ...
                for line in lines[istart:]:
                    if not line.startswith("Run"):
                        if isinstance(data_scale_factor, float):
                            ldp = LumiDataPoint(line, begin_run, end_run, json_file_name, data_scale_factor)
                        elif isinstance(data_scale_factor, dict) and str(day.year) in data_scale_factor:
                            ldp = LumiDataPoint(line, begin_run, end_run, json_file_name, data_scale_factor[str(day.year)])
                        else:
                            ldp = LumiDataPoint(line, begin_run, end_run, json_file_name)
      ...
      if not plot_multiple_years:
        ...
        for color_scheme_name in color_scheme_names:
            ...
            for type in ["lin", "log"]:
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "rr_peak_lumi_per_day_%s_%d%s%s%s" % \
		    ...
		    if min_val>0.0 :
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "rr_int_lumi_per_day_%s_%d%s%s%s" % \
                         (particle_type_str.lower(), year,
                          log_suffix, file_suffix, file_suffix2))
...
#                                transform = ax.transAxes, fontsize=15)
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "rr_int_lumi_per_day_cumulative_%s_%d%s%s%s" % \
                         (particle_type_str.lower(), year,
                          log_suffix, file_suffix, file_suffix2))
      ...
      if not plot_multiple_years:
        ...
        for color_scheme_name in color_scheme_names:
            ...
            for type in ["lin", "log"]:
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "rr_peak_lumi_per_week_%s_%d%s%s%s" % \
                         (particle_type_str.lower(), year,
                          log_suffix, file_suffix, file_suffix2))
            ...
            for type in ["lin", "log"]:
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "rr_int_lumi_per_week_%s_%d%s%s%s" % \
                         (particle_type_str.lower(), year,
                          log_suffix, file_suffix, file_suffix2))
		    ...
		    # Add the logo.
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "rr_int_lumi_per_week_cumulative_%s_%d%s%s%s" % \
                            (particle_type_str.lower(), year,
                             log_suffix, file_suffix, file_suffix2))
        ...
        def PlotAllYears(lumi_data_by_day_per_year, mode):
            ...
            for color_scheme_name in color_scheme_names:
                ...
                for type in ["lin", "log"]:
                    ...
                    if is_log:
                        log_suffix = "_log"
                    SavePlot(fig, "rr_int_lumi_cumulative_%s_%d%s%s%s" % \
            ...
            for type in ["lin", "log"]:
                ...
                for do_cutouts in cutout_settings:
                    ...
                    for do_certification in certification_settings:
                        ...
                        SavePlot(fig, "rr_int_lumi_allcumulative_%s%s%s%s%s%s" % \
...
#                TweakPlot(fig, ax, (time_begin, time_end),
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "rr_peak_lumi_%s%s%s%s" % \
                         (particle_type_str.lower(),
                          log_suffix, file_suffix, file_suffix2))
