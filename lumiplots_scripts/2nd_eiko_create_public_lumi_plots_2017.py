...
class LumiDataPoint(object):
    ...
    def __init__(self, line, run_list, json_file_name=None, data_scale_factor=1):

        # Decode the comma-separated line from lumiCalc.
        begin_run = min(run_list)
        end_run = max(run_list)
...
#        if self.run_number in run_list:
        if (self.run_number >= begin_run) and (self.run_number <= end_run):
                ...
                for line in lines[istart:]:
                    if not line.startswith("Run"):
                        if isinstance(data_scale_factor, float):
                            ldp = LumiDataPoint(line, ListOfRuns, json_file_name, data_scale_factor)
                        elif isinstance(data_scale_factor, dict) and str(day.year) in data_scale_factor:
                            ldp = LumiDataPoint(line, ListOfRuns, json_file_name, data_scale_factor[str(day.year)])
                        else:
                            ldp = LumiDataPoint(line, ListOfRuns, json_file_name)
      ...
      if not plot_multiple_years:
        ...
        for color_scheme_name in color_scheme_names:
            ...
            for type in ["lin", "log"]:
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "eiko_peak_lumi_per_day_%s_%d%s%s%s" % \
		    ...
		    if min_val>0.0 :
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "eiko_int_lumi_per_day_%s_%d%s%s%s" % \
                         (particle_type_str.lower(), year,
                          log_suffix, file_suffix, file_suffix2))
...
#                                transform = ax.transAxes, fontsize=15)
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "eiko_int_lumi_per_day_cumulative_%s_%d%s%s%s" % \
                         (particle_type_str.lower(), year,
                          log_suffix, file_suffix, file_suffix2))
      ...
      if not plot_multiple_years:
        ...
        for color_scheme_name in color_scheme_names:
            ...
            for type in ["lin", "log"]:
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "eiko_peak_lumi_per_week_%s_%d%s%s%s" % \
                         (particle_type_str.lower(), year,
                          log_suffix, file_suffix, file_suffix2))
            ...
            for type in ["lin", "log"]:
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "eiko_int_lumi_per_week_%s_%d%s%s%s" % \
                         (particle_type_str.lower(), year,
                          log_suffix, file_suffix, file_suffix2))
		    ...
		    # Add the logo.
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "eiko_int_lumi_per_week_cumulative_%s_%d%s%s%s" % \
                            (particle_type_str.lower(), year,
                             log_suffix, file_suffix, file_suffix2))
        ...
        def PlotAllYears(lumi_data_by_day_per_year, mode):
            ...
            for color_scheme_name in color_scheme_names:
                ...
                for type in ["lin", "log"]:
                    ...
                    if is_log:
                        log_suffix = "_log"
                    SavePlot(fig, "eiko_int_lumi_cumulative_%s_%d%s%s%s" % \
            ...
            for type in ["lin", "log"]:
                ...
                for do_cutouts in cutout_settings:
                    ...
                    for do_certification in certification_settings:
                        ...
                        SavePlot(fig, "eiko_int_lumi_allcumulative_%s%s%s%s%s%s" % \
...
#                TweakPlot(fig, ax, (time_begin, time_end),
                ...
                if is_log:
                    log_suffix = "_log"
                SavePlot(fig, "eiko_peak_lumi_%s%s%s%s" % \
                         (particle_type_str.lower(),
                          log_suffix, file_suffix, file_suffix2))
